function h = movisensPlotDays(h, start, plots, days, overlap, vals, tsl, tsr, tsm, liml, limr)
...
strIdx = start;
start  = datenum(tsl.Time(1));%datenum(vals{1,strIdx});
...
for iPlot = 1:plots
    ...
    % Find matching day in the subjective data.
    if (iPlot ~= 1)
    
        for jDay = 2:size(vals, 2)
            
            % t2 is the morning date of plotted data
            dataDate = datestr(t2, 'dd-mm-yy');

            % Get morning date of vals.
            valsDate = datestr(vals{8, jDay}, 'dd-mm-yy');

            % Loop through all days in the Vals to find the sleep vars for 
            % the day of the plot. 
            % Why? Sometimes, there is data, but no sleep diary and
            % therefore no estimation of sleep.
            if strcmpi(dataDate, valsDate)
